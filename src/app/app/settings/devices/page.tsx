...
function DeviceRegisterFallback() {
  return (
    <Card className="overflow-hidden">
      ...
      <CardContent className="grid gap-3">
        {[0, 1, 2].map((item) => (
          ...
          >
            <span className="block h-4 w-2/3 animate-pulse rounded-sm bg-surface-3" />
            <span className="block h-3 w-1/3 animate-pulse rounded-sm bg-surface-3" />
          </div>
        ))}
      </CardContent>
    </Card>
          ...
          <table className="w-full text-[0.8125rem]">
            ...
            <tbody>
              {devices.map((device) => (
                ...
                >
                  <td className="px-4 font-medium text-foreground">{device.name}</td>
                  <td className="px-4">
                  ...
                  <td className="px-4">
                    <StatusChip
                      label={device.status}
                      tone={device.status === "ACTIVE" ? "benign" : "inconclusive"}
                    />
                  </td>
                  <td className="px-4">
                    <Datum className="text-muted">{formatDate(device.createdAt)}</Datum>
                  </td>
...
export default async function DevicesPage({
  ...
  return (
    <div className="grid gap-8">
      <PageHeader
        ...
        actions={
          <Button asChild variant="secondary" size="sm">
            <a href="/docs/device-ingestion-api.md" target="_blank" rel="noopener">
      ...
      <Suspense fallback={<DeviceRegisterFallback />}>
        <DeviceRegister />
      </Suspense>
    </div>
  );
